...
const VALIDATOR_TYPE_NO_SPACES = "NO_SPACES";
...
export const VALIDATOR_NO_SPACES = () => ({ type: VALIDATOR_TYPE_NO_SPACES });
...
export const validate = (
  ...
  for (const validator of validators) {
    ...
    if (validator.type === VALIDATOR_TYPE_MINLENGTH) {
      isValid =
        isValid && value.toString().trim().length >= (validator.val || 0);
    }
    ...
    if (validator.type === VALIDATOR_TYPE_NO_SPACES) {
      isValid = isValid && !/\s/.test(value.toString());
    }
  }
  return isValid;
};
